...
class Time extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: vstack ? CrossAxisAlignment.center : CrossAxisAlignment.end,
      children: [
        Container(height: vstack ? 3 : 0.8),
        Row(children: [
          Text(
            "${(prediction.isApp == "1") ? 'Now' : formatTime(convertShortColor(prediction.arrT), prediction.prdt, vstack)}",
            style: TextStyle(fontSize: smallCard ? 22 : 26, fontWeight: FontWeight.w600),
          ),
          if ((prediction.isApp == "0" &&
                  formatTime(convertShortColor(prediction.arrT), prediction.prdt, vstack) !=
                      "Late") &&
              formatTime(convertShortColor(prediction.arrT), prediction.prdt, vstack) != "Now") ...[
            Container(width: 5),
